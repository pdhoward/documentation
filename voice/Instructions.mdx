...
Short answer: these limits are **safe and workable for a pilot hotel widget**, but I’d loosen a couple of them for a real busy resort and rely on your existing abuse controls to protect you.
...
* `RATE_USER_PER_MIN` doesn’t matter – widget calls don’t use the console JWT cookie.
* `RATE_IP_PER_MIN=60` means: from one IP, you’ll allow **60 hits to `/api/session` per minute**.
...
* That’s already *very* liberal for a hotel website. It’s extremely unlikely they’ll hit that with legitimate traffic.

**Caveat:** if in the future you start using `/api/session` for more frequent operations (not just session creation), you might want to bump this to something like `120` or `180`. But for “one POST per call start”, `60` is fine.
...
This is effectively: **per tenant**, you’ll allow 12 session creations per minute.

* That’s 1 new voice call every ~5 seconds.
* For a hotel/resort, that’s *already pretty generous*.
* If traffic grows to “crazy busy”, you can bump this to e.g. `30`, but 12 is not going to clip normal behavior.

👉 I’d keep `12` to start. It’s a good “are we under attack or not?” threshold.
...
If you want to start even more conservatively, keep 10 for “free/trial” plans and 20+ for paying customers.
...
* Let’s say one typical booking conversation is ~5 minutes of back-and-forth.
* That might be ~800–1500 tokens total (hugely approximate, but good enough).
...
* If they’re bigger / chain-level / heavy marketing, they *could* hit that.
...
If you want the hotel widget to feel “always on” and let the **$ limit be your real safety valve**, you could:
...
* Under normal usage, they’ll never hit the token limit.
...
If you’re more conservative on cost, your current `300k` is okay; just know it’s “few hundred conversations / day” territory.
...
* 5 minutes idle timeout is also fine – user walks away, closes tab, etc.
...
* “Let people demo it, but if someone tries to hammer it, shut it down fast.”
...
**Yes**, with two tweaks I’d strongly recommend:
...
1. **Increase concurrency for widget:**
   ...
   so a busy resort doesn’t get “agent busy” at the first little spike.
...
2. **Consider loosening the token cap and letting the $ cap be the real fuse:**
   ...
   – or keep `300000` tokens for now, but know you might increase it once you see real usage.
...
you’re in a good place: **hotel widget feels always-available for normal traffic, but a single bad actor or script can only do limited damage before tripping a fuse.**


For widgets, the right mental model is:

Tenant should feel “unlimited”

…but you still enforce fairness + safety at three levels:
...
maxSessionIdleSec
These are your best “abuse-proof” controls because real users don’t talk for 2 hours nonstop.

✅ Keep these.

B) Per-tenant concurrency (business safety)

A hotel doesn’t need 200 concurrent calls from one page unless something is wrong.
You already show:

active / maxConcurrent per tenant

✅ This is the best non-disruptive limiter: it protects cost without affecting typical customers.

C) Per-visitor “behavioral” detection (abuse safety) — not blunt rate limiting

Instead of “requests/min”, detect patterns like:

very long sessions repeatedly

repeated sessions from same IP/device across many tenants

repeated “start session” without meaningful usage

extremely high token burn vs typical for that agent (your “predictable use case” point)

What action do you take when detected?

Use “step-up friction”, not a hard block:
...
tenant:<tenantId>:ip:<ip>:<winId> → session starts per minute

ip:<ip>:<winId> → session starts per minute across all tenants

tenant:<tenantId>:ip:<ip>:day:<YYYY-MM-DD> → starts per day

tenant:<tenantId>:ip:<ip>:overdur:<YYYY-MM-DD> → “over-duration sessions” count (if you want)
...
if risk high → respond with 403 and code: "STEP_UP_REQUIRED" plus a stepUp: { kind: "captcha" | "otp" }
...
Session “liveness” states (what the dashboard should show)

We should separate “tracking flag” from “liveness.”
...
idleSec <= staleGraceSec (ex: 1 hour or 24 hours — your choice)
...
optionally show endedAt/reason in an “Ended sessions” view

Right now you only show 1–3, but your dataset contains a ton of #3.

1) Who is being measured by idleSec / staleGraceSec?

That “digit” (idle seconds / age seconds) is per session record, i.e. per visitor conversation session (a single Realtime session identified by smSessionId), not the tenant itself.
...
idleSec = “how long since that visitor’s session sent heartbeat”

staleGraceSec = “after how long without heartbeat do we consider that visitor session abandoned/stale”
...
Tenants can iterate on agents, PR their changes, review history, branch, etc. That’s exactly what Git is good at.
...
Each tenant’s agent spec only affects that tenant. “Jailbreaks” in their prompts only hurt their own bot, which is exactly what they’re paying you to control.

Things to watch out for

Runtime fragility
If you fetch from GitHub on every connect, you’re now dependent on GitHub latency and outages.
→ Fix: cache the parsed result per tenantId + agentId (e.g., in Redis or your own DB) and refresh on a timer or when webhook says “new commit”.

Validation
A tenant can easily break the file (missing YAML, typo in heading, etc.).
→ Fix: validate after parsing and:
...
Security
You’ll probably want to restrict which repos/branches are allowed, and make sure you only fetch from whitelisted hosts.

Given those caveats, I’d absolutely go this route. The key is: treat AGENT_ID.md as a source artifact, but cache + validate before it ever reaches Realtime.
